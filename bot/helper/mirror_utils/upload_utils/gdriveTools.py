...
class GoogleDriveHelper:
    ...
    def deletefile(self, link: str):
        ...
        try:
            ...
            LOGGER.info("Delete Result: Successfully deleted")
        except HttpError as err:
            if "File not found" in str(err):
                msg = "☹️ 𝐍𝐨 𝐬𝐮𝐜𝐡 𝐟𝐢𝐥𝐞 𝐞𝐱𝐢𝐬𝐭"
            elif "insufficientFilePermissions" in str(err):
                msg = "☹️ Insufficient File Permissions"
            else:
                msg = str(err)
    ...
    def clone(self, link):
        ...
        try:
            ...
            if mime_type == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                buttons = ButtonMaker()
                durl = short_url(durl)
                buttons.buildbutton("☁️ 𝗗𝗥𝗜𝗩𝗘 𝗟𝗜𝗡𝗞 ☁️", durl)
                ...
                if mime_type is None:
                    mime_type = "File"
                msg += (
                    f'\n├─🕹️ 𝗦𝗶𝘇𝗲 ⇢ {get_readable_file_size(int(meta.get("size", 0)))}'
                )
                msg += f"\n├─💻 𝐓𝐲𝐩𝐞 ⇢ {mime_type}"
                if INDEX_URL is not None:
                    ...
                    if VIEW_LINK:
                        ...
                        urls = short_url(urls)
                        buttons.buildbutton("🌐 𝗩𝗜𝗘𝗪 𝗟𝗜𝗡𝗞 🌐", urls)
    ...
    def drive_list(
        ...
        for index, parent_id in enumerate(DRIVES_IDS):
            ...
            if not Title:
                msg += (
                    f'<h4>{len(response["files"])} ✅𝐒𝐞𝐚𝐫𝐜𝐡 𝐑𝐞𝐬𝐮𝐥𝐭 𝐅𝐨𝐫: {fileName}</h4>'
                )
                Title = True
            ...
            for file in response.get("files", []):
                ...
                if mime_type == "application/vnd.google-apps.folder":
                    ...
                    msg += (
                        f"📂 𝐅𝐢𝐥𝐞𝐧𝐚𝐦𝐞 ⇢ <code>{file.get('name')}<br>(folder)</code><br>"
                    )
                    furl = short_url(furl)
                    msg += f"<b><a href={furl}>☁️ 𝗗𝗥𝗜𝗩𝗘 𝗟𝗜𝗡𝗞 ☁️</a></b>"
                    ...
                    if INDEX_URLS[index] is not None:
                        ...
                        if VIEW_LINK:
                            ...
                            urls = short_url(urls)
                            msg += f' <b>| <a href="{urls}">🌐 𝗩𝗜𝗘𝗪 𝗟𝗜𝗡𝗞 🌐</a></b>'
                msg += "<br><br>"
                contents_count += 1
        ...
        buttons = ButtonMaker()
        buttons.buildbutton("🔎 𝗩𝗜𝗘𝗪 ", f"https://telegra.ph/{path[0]}")
