...
def _clone(message, bot, multi=0):
    ...
    if FSUB:
        try:
            ...
            if user.status not in ("member", "creator", "administrator", "supergroup"):
                ...
                Thread(
                    target=auto_delete_upload_message, args=(bot, message, msg)
                ).start()
                return
        except Exception:
            pass
    ...
    if is_gdtot:
        try:
            msg = sendMessage(
                f"𝐁𝐘𝐏𝐀𝐒𝐒𝐈𝐍𝐆 𝐆𝐃𝐓𝐎𝐓 𝐋𝐈𝐍𝐊 ⇝ <code>{link}</code>", bot, message
            )
            link = gdtot(link)
            deleteMessage(bot, msg)
        except DirectDownloadLinkException as e:
    ...
    if is_gdrive_link(link):
        ...
        if files <= 20:
            msg = sendMessage(
                f"⚙️ 𝐂𝐥𝐨𝐧𝐢𝐧𝐠 𝐘𝐨𝐮𝐫 𝐅𝐢𝐥𝐞/𝐅𝐨𝐥𝐝𝐞𝐫 𝐈𝐧𝐭𝐨 𝐌𝐲 𝐃𝐫𝐢𝐯𝐞 !! 𝐘𝐨𝐮𝐫 𝐋𝐢𝐧𝐤 ⇝ <code>{link}</code>",
                bot,
                message,
            )
        ...
        Thread(
            target=auto_delete_upload_message, args=(bot, message, uploadmsg)
        ).start()
        if MIRROR_LOGS:
            try:
